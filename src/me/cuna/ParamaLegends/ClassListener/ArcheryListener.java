...
import me.cuna.ParamaLegends.Spells.Archery.*;
...
public class ArcheryListener implements Listener{
            ...
            switch(item.getItemMeta().getDisplayName()){
                case "§aTotsuka's Creation" -> {
                    if (player.isNotSilenced())
                        if(player.checkLevel( 3, ClassType.ARCHERY))
                            totsukaCreation.castSpell(player);
                }
                case "§aWind Boost" -> {
                    if (player.isNotSilenced())
                        if(player.checkLevel( 4, ClassType.ARCHERY))
                            windBoost.castSpell(player);
                }
                case "§aSoulstring" -> {
                    if (player.isNotSilenced())
                        if(player.checkLevel( 6, ClassType.ARCHERY))
                            soulstring.castSpell(player);
                }
                case "§aRoyal Artillery" -> {
                    if (player.isNotSilenced())
                        if(player.checkLevel( 9, ClassType.ARCHERY))
                            royalArtillery.castSpell(player);
                }
                case "§aHuayra's Fury"-> {
                    if (player.isNotSilenced())
                        if(player.checkLevel( 7, ClassType.ARCHERY))
    ...
    public void shootArrow(PlayerParama playerParama, EntityShootBowEvent event, String name){
        switch(name){
            ...
            case "§aViper's Bite" -> {
                if(playerParama.isNotSilenced()){
                    if(playerParama.checkLevel( 2, ClassType.ARCHERY)){
            ...
            case "§aNeurotoxin" -> {
                if(playerParama.isNotSilenced()){
                    if(playerParama.checkLevel( 5, ClassType.ARCHERY)){
            ...
            case "§aRetreat" -> {
                if(playerParama.isNotSilenced()){
                    if(playerParama.checkLevel( 7, ClassType.ARCHERY)){
            ...
            case "§aBlast" -> {
                if(playerParama.isNotSilenced()){
                    if(playerParama.checkLevel( 8, ClassType.ARCHERY)){
            ...
            case "§aWhistling Wind" -> {
                ...
                if(playerParama.isNotSilenced()){
                    if(playerParama.checkLevel(10, ClassType.ARCHERY)){
